...
- 0～255のパネル番号に割り当てられた値(BVEはこの各パネル番号に格納された値によって画面内のパイロットランプや各種表示灯やメーターなどを動かしており、車両データ作者様によってこの割り当ては異なります、ATSなどの保安機器や信号動作も制御できることからATSプラグインのATSパネル番号などと表現することもあります)
- 速度、各圧力値、電圧、電流値等
- 各マスコン、ブレーキ、レバーサ情報  
など多岐に渡ります。  
